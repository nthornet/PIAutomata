...
def drawGrid():
    ...
    for img in cutimg:
        ...
        try:
            ...
            x = img.position[1]-1
            y = img.position[0]-1
            celula = entities.Celula(x,y,imagen, outfile)
            Automata[x,y] = celula
        except:
            t=1
...
def colorize(item, life_dict):
    ...
    coordinates = diccionario_cords[cel.path]
...
def getNeighbours(item, life_dict):
    ...
    for x in range(-1, 2):
        for y in range(-1, 2):
            ...
            if neighbour[0] < WIDTH/CELL and neighbour[0] >= 0:
                if neighbour[1] < HEIGHT/CELL and neighbour[1] >= 0:
                    try:
